...
export default function CheckoutPage() {
  ...
  const effectivePeople = Number(booking.people) || 1;
  const subtotal = cartItems.reduce((sum, i) => sum + parseFloat(i.price) * i.qty, 0);
  const deliveryCharge = 0;
  const total = (subtotal * effectivePeople) + deliveryCharge;
  ...
  const handleCheckout = async () => {
    ...
    const orderPayload = {
      ...
      discount: "0",
      coupon: "",
      // ── booking filters in payload ──
      deliveryDate: booking.date || "",
      numberOfPeople: booking.people ? String(booking.people) : "",
      budgetPerPerson: booking.budget || "",
      deliveryTime: new Date().toLocaleTimeString("de-DE", {
        hour: "2-digit",
        minute: "2-digit",
      }),
      deliveryTerm: "",
      ...
      items: cartItems.map((item) => ({
        ...
        // qty = dish qty × people so invoice shows correct count
        qty: String(Number(item.qty) * effectivePeople),
      })),
    };
    ...
    if (checkoutThunk.fulfilled.match(result)) {
      ...
      if (orderNo) {
        dispatch(invoiceEmailThunk({ orderNo, order_type: "regular" }));
      }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-white rounded-2xl shadow-sm p-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <input name="street" value={form.street} onChange={handleChange} placeholder="Street name"
                    className="border border-gray-200 rounded-xl p-3 text-gray-800 text-sm focus:ring-2 focus:ring-purple-400 outline-none" />
                </div>

                <div className="flex flex-col gap-1">
                  <label className="text-xs text-gray-500 font-semibold uppercase tracking-wide">Street No</label>
                  <input name="streetNo" value={form.streetNo} onChange={handleChange} placeholder="Street number"
                    className="border border-gray-200 rounded-xl p-3 text-gray-800 text-sm focus:ring-2 focus:ring-purple-400 outline-none" />
                </div>

                <div className="flex flex-col gap-1">
                  <label className="text-xs text-gray-500 font-semibold uppercase tracking-wide">House No</label>
                  <input name="houseNumber" value={form.houseNumber} onChange={handleChange} placeholder="House number"
                    className="border border-gray-200 rounded-xl p-3 text-gray-800 text-sm focus:ring-2 focus:ring-purple-400 outline-none" />
                </div>

                <div className="flex flex-col gap-1">
                  <label className="text-xs text-gray-500 font-semibold uppercase tracking-wide">Floor No</label>
                  <input name="floorNo" value={form.floorNo} onChange={handleChange} placeholder="Floor"
                    className="border border-gray-200 rounded-xl p-3 text-gray-800 text-sm focus:ring-2 focus:ring-purple-400 outline-none" />
                </div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-2xl shadow-sm p-6 sticky top-24">
              ...
              {/* ── NEW: Booking info chips in checkout ── */}
              {(booking.date || booking.people || booking.budget) && (
                <div className="flex flex-wrap gap-2 mb-4 pb-4 border-b border-gray-100">
                  {booking.date && (
                    <span className="flex items-center gap-1.5 text-xs bg-purple-50 text-purple-700 border border-purple-200 px-2.5 py-1.5 rounded-full font-medium">
                      <Calendar size={11} />
                      {booking.date}
                    </span>
                  )}
                ...
                <>
                  <div className="border-t mt-4 pt-4 flex flex-col gap-2 text-sm">
                    <div className="flex justify-between text-gray-500">
                      <span>Subtotal <span className="text-xs">(per person)</span></span>
                      <span>€{subtotal.toFixed(2)}</span>
                    </div>
                    {effectivePeople > 1 && (
                      <div className="flex justify-between text-purple-600 text-xs font-medium">
                        <span>× {effectivePeople} people</span>
                        <span>€{(subtotal * effectivePeople).toFixed(2)}</span>
                      </div>
                    )}
                    <div className="flex justify-between text-gray-500">
                      <span>Delivery</span>
